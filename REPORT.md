...
>
> **기준선 도구 83.3% / 답변 58.3% → 19회차 골든셋 100%/100% · 검증셋 29/29 · 안정성 12/12**
>
...
| **#19** | `data/holdout.json`<br>`evaluate_holdout.py` | 12건은 **18회차 동안 보면서 고친 셋**이다. 100% 가 «진짜 잘하게 됐다》 인지 «그 12건에만 맞춰졌다》 인지 구분이 안 된다 — 같은 시험지를 18번 보고 100점 받은 것과 같다 | 검증셋 29건(경계 8·애매 8·제품명 8·대화 5). 채점 방식을 달리했다 — **경로(답변·되묻기·넘김) 채점**, 도구는 **부분집합**, `turns` 는 마지막 턴 채점 | **29/29** | **29/29** | **첫 측정의 실패 1건이 «내 정답이 틀린 것»** 이었다. 「캐비어」 에 답변을 정답으로 박았는데 봇이 5/5 되물었다 — *"어떤 어종(철갑상어/연어)인지"*. **연어알도 캐비어라 부르고 철갑상어만 CITES 다. 봇이 맞고 내가 틀렸다.**<br>안정성 28/29 — 흔들린 것은 **B2**, #11 에서 *"거기서 멈췄다"* 고 적은 그 문항이다. **검증셋이 독립적으로 같은 약점을 짚었다**<br>⚠️ **29건 중 상당수는 이미 보면서 고쳤다**(#8~#17). 엄밀한 holdout 이 아니라 «개선용 2호» 다 |
...
**손댄 곳의 성격**: 문서 5 · 검색 3 · 프롬프트 5 · 구조 6 · 모델 2 · 형식 1 · 측정 3.
**케이스별 코드 관문은 한 개도 넣지 않았다.**

> 측정 도구를 고친 것이 **세 번**이다 — #0 직전(채점기·가드레일), #18(분산), #19(검증셋).
> 세 번 다 **잘못 재고 있었다는 것을 알아차린** 회차이고, 세 번 다 점수를 올리지 않았다.
> 그래도 이 세 번이 나머지 16회차를 믿을 수 있게 만들었다.
...
12건은 **개선용이 되어 버렸다.** 그래서 `data/holdout.json` 29건을 따로 만들어 쟀다.

| 지표 | 결과 |
|---|---|
| 경로 적절성 (답변·되묻기·넘김) | **29/29 = 100%** |
| 도구 호출 적절성 (필수 도구 포함) | **29/29 = 100%** |
| 답변 적절성 | **29/29 = 100%** |
| 안정성 (2회) | **28/29** — 흔들린 문항 `B2` |
| 묶음별 | 경계 8/8 · 애매 8/8 · 제품명 8/8 · 대화 5/5 |

**만든 날 정답 1건이 틀렸다.** 「캐비어」 에 «답변 + lookup_cites» 를 정답으로 박았는데
봇이 5/5 되물었다 — *"어떤 어종(철갑상어/연어 등)인지"*. **연어알(이크라)도 캐비어라 부르고
철갑상어만 CITES 대상이다.** 봇이 맞고 내가 틀렸다. #0 직전에 채점기가 틀려 있었던 것과 같은 일이다.

**한계: 29건 중 상당수는 이미 보면서 고쳤다** (#8~#17 이 그 질문들로 고친 회차다).
엄밀한 holdout 이 아니라 **«개선용 2호»** 에 가까워서, 100% 를 «과적합이 아니다》 의 증거로 쓸 수 없다.
진짜 검증은 **다른 사람이 만든 질문**으로 해야 한다 — `data/holdout_외부.json` 자리를 비워 뒀다.

**12건은 이제 개선용이 되어 버렸다.** 같은 셋을 보고 18회 고친 결과이므로 과적합일 수 있다.
...
| | |
|---|---|
| **`data/holdout_외부.json`** | **1순위.** 강사님·동료에게 질문 10건을 받아 넣고 **한 번만** 잰다. 내가 만들면 내가 아는 범위에서만 나온다 — 강사님이 «평가셋은 사람이 직접 만들라» 고 한 이유가 이것이다 |
| **B2 (입국장면세점 위스키)** | 검증셋에서 흔들린 유일한 문항. 프롬프트가 아니라 문서 쪽을 볼 차례다 — 「공제 순서」 를 절로 따로 떼면 나아질 수 있다 |
