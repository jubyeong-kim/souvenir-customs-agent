...
| #2 | 검색에 IDF | 도구 | 91.7% | 75.0% |
| #3 | 규정 문서 우선, 사례집은 보조 | 도구 | 91.7% | 83.3% |
| #4 | 분류에 "방향" 규칙 한 줄 | 프롬프트 | **100%** | 91.7% |
| #5 | 나란히 놓인 두 경우를 둘 다 | 프롬프트 | **100%** | **100%** |

각 회차는 **한 군데만** 고치고 **최소 2회** 쟀다. 마지막은 3회 연속 만점.
회차별 상세는 [실험기록.md](실험기록.md).

**#2 — 흔한 말이 점수를 지배했다.** 통관 문서에서 `신고` 는 거의 모든 절에 나와 **절을 구분하지 못하는데**
빈도 합계가 점수라서 가장 크게 기여했다. 정작 「가산세 40%·60%」 절은 7위로 밀려 답변이 *"자료에 없습니다"* 가 됐다.
처음엔 절 길이를 의심했지만 분포가 343~770자로 고르게 나와, 문제는 길이가 아니라 **어휘**였다.

**#3 — 수(數)로 밀린다.** 면세 카테고리에 사례집 10절 vs 규정 3절이라, 점수와 무관하게 상위 3자리를 사례집이 채웠다.
사례집은 보충 설명이지 1차 근거가 아니므로 규정 문서를 먼저 채우게 했다.
**여기서 회귀를 하나 만들었다** — 무조건 한 자리를 비워 두었더니 보조 문서가 없는 카테고리(검역)의 근거가
3절 → 2절로 줄어 Q2 가 깨졌다. `imp3-1` 의 83.3% 만 봤다면 "좋아졌다"고 넘어갔을 것이다.
**목록에 없던 Q2 가 새로 들어온 것**이 보여서 잡았다 — 점수가 아니라 실패 목록으로 비교한 덕이다.

**#4·#5 — 프롬프트 한 줄씩.** 「홍삼을 미국 갈 때」를 품목만 보고 `검역` 으로 분류하던 것을
*"한국에서 나가는 이야기는 물건이 무엇이든 범위밖"* 한 줄로 고쳤고, 「미신고 40%」는 쓰면서 바로 옆
「자진신고 30% 경감」을 빠뜨리던 것을 *"나란히 놓인 두 경우는 둘 다 쓴다"* 로 고쳤다.

### 손댄 곳의 성격

문서 1 · 검색 2 · 프롬프트 2. **케이스별 코드 관문은 한 개도 넣지 않았다.**
지난 과제에서 27회를 고친 이유가 케이스마다 코드 관문을 쌓은 것이었고,
5회로 끝낸 동료는 프롬프트의 일반 규칙을 먼저 박았다. 이번에는 그 순서를 지켰다.

### 100% 를 어떻게 읽어야 하나

**12건은 작고, 같은 셋을 보고 고친 결과다. 과적합일 수 있다.**

다만 고친 것들이 케이스별 땜질이 아니라는 점은 말할 수 있다 — IDF, 규정 문서 우선, 방향 규칙,
나란한 두 경우. 넷 다 이 12건 밖에서도 작동하는 **일반 규칙**이다.
그래도 확인하려면 **고치는 동안 한 번도 보지 않은 문항으로 다시 재야 한다.** 다음 작업의 1순위다.
