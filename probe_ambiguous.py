"""어려운 문의를 돌려 본다. 채점하지 않는다 — 어디로 가는지 보는 것이 목적이다.

    python probe_ambiguous.py            # 전부
    python probe_ambiguous.py 경계        # 분류가 걸치는 것만
    python probe_ambiguous.py 애매        # 되물어야 하는 것만
    python probe_ambiguous.py 제품명      # 브랜드·제품명으로 묻는 것만

평가셋 12건은 카테고리가 깔끔하게 갈리고 필요한 정보가 다 들어 있는 문항만 있다.
실제 문의는 두 영역에 걸치거나, 답을 정하는 정보가 빠져 있다.
"""
import sys

import agent

BOUNDARY = [
    ("악어가죽 지갑도 800달러 안에 들어가면 그냥 가져올 수 있나요?",
     "멸종위기종 ↔ 면세  — 금액 이야기를 하지만 핵심은 CITES 허가다"),
    ("입국장면세점에서 산 위스키도 술 면세 한도에 포함되나요?",
     "면세점 ↔ 면세  — 산 곳은 면세점, 묻는 것은 주류 한도"),
    ("독일 소시지 30달러어치 샀는데 면세 한도 안이면 괜찮죠?",
     "면세 ↔ 검역  — 금액은 상관없고 검역이 막는다"),
    ("캐비어를 선물로 받았는데 가져올 수 있나요?",
     "검역 ↔ 멸종위기종  — 식품이면서 철갑상어(CITES)다"),
    ("화장품에 사향 성분이 들어있다는데 문제 될까요?",
     "멸종위기종 ↔ 면세  — 일반 화장품처럼 들리지만 CITES 성분이다"),
    ("해외 쇼핑몰에서 주문해서 택배로 받는 술은 몇 병까지 되나요?",
     "면세 ↔ 범위밖  — 해외직구(발송)는 이 봇의 범위가 아니다"),
    ("반려견을 데리고 입국하려면 뭐가 필요한가요?",
     "검역 ↔ 범위밖  — 기념품은 아니지만 동물검역 문서에 있다"),
    ("면세 한도 넘은 걸 공항에서 그냥 버리고 나와도 되나요?",
     "면세 ↔ 범위밖  — 폐기 절차는 우리 문서에 없다"),
]


# 사람이 애매하게 말해서 **한 번 더 물어야** 하는 문의.
# 빠진 정보가 답을 뒤집는다 — 짐작해서 답하면 그럴듯하게 틀린다.
# (chatbot/PLAYBOOK.md 3-③: 식별자 없으면 도구 호출 금지 + 되묻기 강제)
VAGUE = [
    ("이거 세금 내야 하나요?",
     "품목도 금액도 없다 — 무엇을, 얼마에 샀는지 없이는 아무 답도 못 한다"),
    ("술 좀 사왔어요.",
     "용량과 금액이 없다 — 2L·400달러 둘 다 따져야 하는데 둘 다 모른다"),
    ("고기 좀 사왔는데 괜찮아요?",
     "어느 나라에서, 검역증명서가 있는지에 따라 가능/불가가 정반대다"),
    ("가방 하나 샀어요. 신고해야 되나요?",
     "금액을 모르고, 재질이 악어가죽이면 CITES 로 이야기가 달라진다"),
    ("한약재 가져와도 돼요?",
     "웅담·사향이면 CITES, 일반 약재면 검역 — 품목명이 없으면 갈 수 없다"),
    ("동물 가죽으로 만든 제품인데 문제 있을까요?",
     "어떤 동물인지가 전부다. 소가죽과 악어가죽이 다르다"),
    ("친구가 부탁해서 대신 사온 것도 면세되나요?",
     "자가사용이 아니면 면세 기준이 적용되지 않는다 — 수량·용도를 물어야 한다"),
    ("면세 한도 넘었는데 어떻게 해요?",
     "이미 입국했는지, 아직 신고 전인지에 따라 30% 경감과 40% 가산세로 갈린다"),
]

# 사람은 품목 이름이 아니라 **제품명·브랜드**로 말한다.
# 우리 사전(SYNONYMS·CROSS_CHECK)은 「육포」 는 알지만 「비첸향」 은 모른다.
# 브랜드를 사전에 적어 넣는 것은 끝이 없다 — 어디까지 일반화되는지 먼저 본다.
BRAND = [
    ("홍콩에서 비첸향 사왔는데 괜찮을까요?",
     "비첸향 = 육포 브랜드 → 검역. 사전에 「육포」는 있지만 「비첸향」은 없다"),
    ("스페인에서 하몽 한 덩어리 사왔어요.",
     "하몽 = 생햄 → 검역. 외래어 품목명이고 사전에 없다"),
    ("맥캘란 18년 두 병 사왔는데 면세되나요?",
     "위스키 → 면세. 브랜드명이지만 「병」 「면세」 로 걸릴 여지가 있다"),
    ("에르메스 크로커딜 백을 하나 샀는데 문제될까요?",
     "크로커딜 = 악어 → CITES. 「악어」 는 사전에 있지만 「크로커딜」 은 없다"),
    ("프랑스에서 푸아그라 통조림 사왔어요.",
     "푸아그라 = 거위 간 가공품 → 검역. 「통조림」 으로 걸릴 여지가 있다"),
    ("태국에서 두리안 사왔는데 되나요?",
     "두리안 = 생과일 → 검역(수입금지). 「두리안」 은 CROSS_CHECK 에 있다 — 대조군"),
    ("차이나타운에서 샥스핀 사왔어요.",
     "샥스핀 = 상어지느러미 → CITES 여부가 종에 따라 갈린다"),
    ("벨기에에서 고디바 초콜릿 20박스 사왔는데 신고해야 하나요?",
     "초콜릿은 검역 대상이 아니다 → 면세 한도 이야기. **과잉 발동 대조군**"),
]

GROUPS = {"경계": BOUNDARY, "애매": VAGUE, "제품명": BRAND}


def main():
    picked = sys.argv[1] if len(sys.argv) > 1 else None
    for name, cases in GROUPS.items():
        if picked and picked != name:
            continue
        bar = "#" * 78
        print(f"\n\n{bar}\n#  {name}\n{bar}")
        run_group(cases)


def run_group(cases):
    for i, (q, why) in enumerate(cases, 1):
        s = agent.run(q)
        print(f"\n{'=' * 78}\n[{i}] {q}\n    어려운 지점: {why}")
        print(f"    분류: {s['category']}   도구: {s['tools_called'] or '없음 (넘김)'}"
              f"   검증: {'위반 ' + str(s['violations']) if s['violations'] else '통과'}")
        for e in s["evidence"]:
            print(f"      근거: {'보조' if e['보조'] else '규정'} · {e['제목'][:52]}")
        print(f"\n{s['answer']}")


if __name__ == "__main__":
    main()
